...
def search_on_spotify(song, token):
    ...
    print(
        f"    \033[92mFound {items['name']} on spotify by {', '.join([artist['name'] for artist in artists])}\033[00m"
    )
    return items


def adding_songs_to_spotify_playlist(name, song_list, token):
    if len(song_list) == 0:
    ...
    spotifyobj.user_playlist_create(
        user=user["id"],
        name=name,
        public=True,
        description="This is generated by a python script",
    )
    ...
    for i in range(len(song_list) // 100 + 1):
        ...
        while True:
            try:
                spotifyobj.user_playlist_add_tracks(
                    user=user["id"], playlist_id=playlist_id, tracks=songs
                )
                break
            except:
                # print in red color
                attempt += 1
                print(
                    f"\033[91mError occured while adding songs to spotify playlist\033[00m Trying again attempt {attempt}..."
                )
                if attempt == 5:
                    break
...
def main():
    ...
    # link validation
    if not re.match(
        r"https://(www.)?youtube.com/playlist\?list=[a-zA-Z0-9_-]+", youtube_playlist
    ):
    ...
    for song, author in get_songs(youtube_playlist):
        count += 1
        print(count, end=". ")
        song_on_spotify = search_on_spotify(song + " " + author, token=token)
        ...
        with open("log.txt", "a", encoding="utf-8") as f:
            f.write(
                f"{song} by {author} --> {song_on_spotify['name']} by {', '.join(artist['name'] for artist in song_on_spotify.get('artists', []))}\n"
            )

    adding_songs_to_spotify_playlist(get_name(youtube_playlist), song_list, token)
    ...
    print(
        f"\033[92m{len(song_list)} Found\033[00m \033[91m{len(song_list) - count} Not found\033[00m"
    )
